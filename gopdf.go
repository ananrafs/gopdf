...
func (gp *GoPdf) MultiCell(rectangle *Rect, text string) error {
	...
	// get lineHeight
	lText, err := gp.curr.FontISubset.AddChars(text)
	if err != nil {
		return err
	}
	_, lineHeight, _, err := createContent(gp.curr.FontISubset, lText, gp.curr.FontSize, gp.curr.CharSpacing, nil)
	if err != nil {
		return err
	}
...
func (gp *GoPdf) MultiCellWithOption(rectangle *Rect, text string, opt CellOption) error {
	...
	// get lineHeight
	lhText, err := gp.curr.FontISubset.AddChars(text)
	if err != nil {
		return err
	}
	_, lineHeight, _, err := createContent(gp.curr.FontISubset, lhText, gp.curr.FontSize, gp.curr.CharSpacing, nil)
	if err != nil {
		return err
	}
